...
def random_filename(length, ext):
	return ''.join([random.choice(string.ascii_lowercase) for i in range(length)]) + '.{}'.format(ext)
...
class StickerDownloader:
	...
	def get_file(self, file_id):
		...
		f = File(name=info['result']['file_path'].split('/')[-1], link='https://api.telegram.org/file/bot{}/{}'.format(self.token, info['result']['file_path']))
	...
	def convert_file(self, _input, _output):
	...
	def convert_to_pngs(self, name):
		...
		with ThreadPoolExecutor(max_workers=self.THREADS) as executor:
			...
			futures = [executor.submit(self.convert_file, _input, os.path.join(png_folder, random_filename(6, 'png'))) for _input in webp_files]
			for i in as_completed(futures):
...
if __name__ == "__main__":
	...
	for sset in names:
		...
		print('-'*60)
		_ = downloader.convert_to_pngs(sset)
